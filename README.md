...
На этой структуре данных можно неплохо реализовать, например, очередь с возможностью выхода любого элемента из нее.
...
Структура данных может использоваться, например, для буферизации. Когда приходят блоки видео-файла их удобно хранить в данной структуре, так как память не переаллоцируется для очередного блока данных. Он особенно хорошо себя показывает, когда данные приходят блоками, размер которых близок к *k*
